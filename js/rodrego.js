...
var fail = new Audio("sound/sound2.mp3");
var success = new Audio("sound/sound3.mp3");
...
// play the various sounds
// looped beep / bleep array so there is no cut-off of sounds
function playBeep() {  
    beeps[thisBeep].play();
    thisBeep++;
...
function runCommands(cmds,i,step) {
    if (cmds[i]['cmd'] != 'end') {
        ...
        // running script (no-step) and 'reset' hasn't been pressed
        if (!step && !reset_on) {
            setTimeout(function() {
